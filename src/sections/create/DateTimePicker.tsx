 ...
 *
 * Rendered as a centered modal (via portal) so it's always positioned in
 * the viewport's middle regardless of where the trigger sits.
 */
export default function DateTimePicker({
  ...
  // Modal in a portal so it always sits dead-center in the viewport.
  const modal =
    open &&
    createPortal(
      <div
      ...
      >
        ...
        >
          ...
          {(() => {
            const allPicked = !!date && !!time && !!timezone;
            return (
              <div className="px-3 pb-3 flex items-center justify-between gap-3">
                <p className="font-display text-[9px] tracking-[0.15em] uppercase text-[#b2c3b1]/45 leading-tight">
                  {allPicked ? '' : 'Pick date, time & timezone'}
                </p>
                <button
                  type="button"
                  onClick={() => allPicked && setOpen(false)}
                  disabled={!allPicked}
                  className={`font-display text-[10px] tracking-[0.2em] uppercase px-4 py-1.5 font-bold transition-colors ${
                    allPicked
