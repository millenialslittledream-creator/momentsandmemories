...
class GalleryApp {
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    if (Math.abs(this.start - x) > 5) this.isDragged = true;
    this.scroll.target = (this.scroll.position ?? 0) + distance;
  }

  onTouchUp(e: MouseEvent | TouchEvent) {
    if (!this.isDragged && this.medias) {
      const clientX = "changedTouches" in e ? e.changedTouches[0].clientX : (e as MouseEvent).clientX;
      const rect = this.container.getBoundingClientRect();
      const normalizedX = ((clientX - rect.left) / rect.width) * 2 - 1;
      const worldX = normalizedX * (this.viewport.width / 2);

      // Find the closest visible card to the click position
      let closestIndex = -1;
      let closestDist = Infinity;
      for (let i = 0; i < this.medias.length; i++) {
        const media = this.medias[i];
        const posX = media.plane.position.x;
        // Only consider cards that are on screen
        if (Math.abs(posX) > this.viewport.width) continue;
        const dist = Math.abs(posX - worldX);
        if (dist < closestDist) {
          closestDist = dist;
          closestIndex = i;
        }
      ...
      if (closestIndex >= 0) {
        // Snap the clicked card to center
        const media = this.medias[closestIndex];
        this.scroll.target = this.scroll.current + media.plane.position.x;
